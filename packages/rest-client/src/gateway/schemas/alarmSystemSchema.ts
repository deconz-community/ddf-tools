...
export const alarmSystemSchema = z.strictObject({
  ...
  name: z.string().min(1).max(32).default('default')
  ...
  config: z.strictObject({
    ...
    disarmed_entry_delay: z.number().min(0).max(255)
      .describe('The delay in seconds before an alarm is triggered.'),
    disarmed_exit_delay: z.number().min(0).max(255)
      .describe('The delay in seconds before the arm mode is armed.'),
    armed_away_entry_delay: z.number().min(0).max(255)
      .describe('The delay in seconds before an alarm is triggered.'),
    armed_away_exit_delay: z.number().min(0).max(255)
      .describe('The delay in seconds before the arm mode is armed.'),
    armed_away_trigger_duration: z.number().min(0).max(255)
      .describe('The alarm trigger duration.'),
    armed_stay_entry_delay: z.number().min(0).max(255)
      .describe('The delay in seconds before an alarm is triggered.'),
    armed_stay_exit_delay: z.number().min(0).max(255)
      .describe('The delay in seconds before the arm mode is armed.'),
    armed_stay_trigger_duration: z.number().min(0).max(255)
      .describe('The alarm trigger duration.'),
    armed_night_entry_delay: z.number().min(0).max(255)
      .describe('The delay in seconds before an alarm is triggered.'),
    armed_night_exit_delay: z.number().min(0).max(255)
      .describe('The delay in seconds before the arm mode is armed.'),
    armed_night_trigger_duration: z.number().min(0).max(255)
      .describe('The alarm trigger duration.'),
  })
  ...
  state: z.strictObject({
    ...
    seconds_remaining: z.number().min(0).max(255)
      .describe('During "exit_delay" and "entry_delay" states, this value holds the remaining time. '
      + 'In all other states the value is 0.'),
  })
    .describe('The current state of the alarm system.'),

  devices: z.record(alarmSystemDeviceSchema)
    .describe('List of devices which are added to the alarm system.'),

}).describe('Configuration of the alarm system.')

export const alarmSystemsSchema = z.record(alarmSystemSchema)
